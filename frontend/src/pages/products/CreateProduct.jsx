...
function CreateProduct() {
    ...
    const generateSKU = () => {
        // Créer un SKU basé sur la catégorie et un nombre aléatoire
        if (product.category) {
            const categoryPrefix = product.category.substring(0, 3).toUpperCase();
            const randomNumber = Math.floor(1000 + Math.random() * 9000); // Nombre à 4 chiffres
            setProduct(prev => ({ ...prev, sku: `${categoryPrefix}-${randomNumber}` }));
        }
    };
        ...
        <div className="max-w-2xl mx-auto p-6">
            ...
            <form onSubmit={handleSubmit} className="space-y-6 bg-white p-6 rounded-lg shadow-md">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        <label className="block text-sm font-medium mb-2 text-gray-600">Quantité initiale</label>
                        <input
                            type="number"
                            name="quantity"
